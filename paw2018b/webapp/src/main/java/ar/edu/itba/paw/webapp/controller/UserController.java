...
public class UserController {
	...
	public Response getUsers(@Context HttpServletResponse response, @NotNull @QueryParam("page") int page,
	...
	public Response createUser(@Context HttpServletRequest request, final UserDTO userDTO) {
		if (!vs.validateUser(userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmail(), userDTO.getPassword(),
		...
		us.create(userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmail(), userDTO.getPassword(),
				userDTO.getPhoneNumber(), request.getHeader("Accept-Language").substring(0, Constants.MAX_LANGUAJE),
				Constants.Role.USER.getRole());
		return rs.create();
	}
	...
	@Path("/users/{userid}")
	@Consumes(value = { UserDTO.MediaType, })
	public Response updateInformation(@Context HttpServletRequest request, @PathParam("userid") long userid,
		...
		if (userDTO.getPassword() != null) {
			...
			if (!vs.validateUserData(userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmail(),
					userDTO.getPhoneNumber()))
				return rs.badRequest("The user parameters are invalid");
	...
	public Response createPublication(@Context HttpServletRequest request, @PathParam("userid") long userid,
			final PublicationDTO publicationDTO) {
		if (!vs.validateID(userid))
			return rs.badRequest("No user found with the specified id");

		if (!vs.validatePublication(publicationDTO.getTitle(), publicationDTO.getAddress(),
				publicationDTO.getNeighborhoodid(), publicationDTO.getCityid(), publicationDTO.getProvinceid(),
				publicationDTO.getOperation(), publicationDTO.getPrice(), publicationDTO.getDescription(),
				publicationDTO.getPropertyType(), publicationDTO.getBedrooms(), publicationDTO.getBathrooms(),
				publicationDTO.getDimention(), publicationDTO.getParking(), publicationDTO.getCoveredFloorSize(),
				publicationDTO.getBalconies(), publicationDTO.getAmenities(), publicationDTO.getStorage(),
				publicationDTO.getExpenses(), tas.getUserIdAuthentication(request)))
			return rs.badRequest("The user parameters are invalid");

		if (tas.getUserIdAuthentication(request) != userid)
			return rs.forbidden("The user has no authority to perform this action");

		Publication pub = ps.create(publicationDTO.getTitle(), publicationDTO.getAddress(),
				publicationDTO.getNeighborhoodid(), publicationDTO.getCityid(), publicationDTO.getProvinceid(),
				publicationDTO.getOperation(), publicationDTO.getPrice(), publicationDTO.getDescription(),
				publicationDTO.getPropertyType(), publicationDTO.getBedrooms(), publicationDTO.getBathrooms(),
				publicationDTO.getDimention(), publicationDTO.getParking(), publicationDTO.getCoveredFloorSize(),
	...
	public Response retrievePublicationInformation(@Context HttpServletRequest request,
			@PathParam("userid") long userid, @PathParam("publicationid") long publicationid) {
		if (!vs.validateID(userid))
			return rs.badRequest("The user id is invalid");
		if (tas.getUserIdAuthentication(request) != userid)
			return rs.forbidden("The user has no authority to perform this action");
	...
	@Path("/users/{userid}/publications/{publicationid}")
	@Consumes(value = { UserDTO.MediaType, })
	public Response updatePublication(@Context HttpServletRequest request, @PathParam("userid") long userid,
			@PathParam("publicationid") long publicationid, final PublicationDTO publicationDTO) {
		if (!vs.validateID(userid))
			return rs.badRequest("The user id is invalid");
		...
		if (!vs.validatePublication(publicationDTO.getTitle(), publicationDTO.getAddress(),
				...
				publicationDTO.getDimention(), publicationDTO.getParking(), publicationDTO.getCoveredFloorSize(),
				publicationDTO.getBalconies(), publicationDTO.getAmenities(), publicationDTO.getStorage(),
				publicationDTO.getExpenses(), publicationid))
			return rs.badRequest("The user parameters are invalid");

		ps.editData(publicationDTO.getTitle(), publicationDTO.getAddress(), publicationDTO.getNeighborhoodid(),
				...
				publicationDTO.getParking(), publicationDTO.getCoveredFloorSize(), publicationDTO.getBalconies(),
	...
	@DELETE
	@Path("/users/{userid}/publications/{publicationid}")
	public Response deletePublication(@Context HttpServletRequest request, @PathParam("userid") long userid,
	...
	public Response sendMessage(MessageDTO messageDTO) {
		if (!vs.validateEmailMessage(messageDTO.getName(), messageDTO.getEmail(), messageDTO.getMessage(),
				messageDTO.getOwnerEmail(), messageDTO.getTitle()))
			return rs.badRequest("The message parameters are invalid");

		MimeMessage email = ms.sendEmail(messageDTO.getName(), messageDTO.getOwnerEmail(), messageDTO.getEmail(),
				messageDTO.getMessage(), messageDTO.getTitle());
		if (email == null)
	...
	@Path("/users/{userid}/publications")
	@Produces(value = { PublicationDTO.MediaType })
	public Response getUserPublications(@Context HttpServletResponse response, @Context HttpServletRequest request,
			@PathParam("userid") long userid, @DefaultValue("0") @QueryParam("page") Integer page,
	...
	@Produces(value = { PublicationDTO.MediaType })
	public Response getUserFavoutirePublications(@Context HttpServletResponse response,
	...
	public Response addFavouritePublication(@Context HttpServletRequest request, @PathParam("userid") long userid,
			final FavPublicationDTO favPublicationDTO) {
		if (!vs.validateID(userid))
			return rs.badRequest("The user id is invalid");
	...
	public Response removeFavouritePublication(@Context HttpServletRequest request, @PathParam("userid") long userid,
			@PathParam("publicationid") long publicationid) {
		if (!vs.validateID(userid))
			return rs.badRequest("The user id is invalid\"");
		if (!vs.validateID(publicationid))
