...
const PrivateRoute = ({component: Component, componentProps, ...rest}) => {
	if (componentProps) {
		...
		return (
			<Route
				{...rest}
				render={(props) => (UserService.isLogged() ? <Component {...props} /> : <Redirect to='/SignUp' />)}
			/>
		);
	}
};
...
const AdminRoute = ({component: Component, ...rest}) => {
	return (
		<Route
			{...rest}
			render={(props) =>
				UserService.isLogged() && UserService.isAdmin() ? <Component {...props} /> : <Redirect to='/' />
			}
		/>
	);
};
...
const NotLockedRoute = ({component: Component, ...rest}) => {
	return (
		<Route
			{...rest}
			render={(props) =>
				UserService.isLogged() && !UserService.isLocked() ? <Component {...props} /> : <Redirect to='/' />
			}
		/>
	);
};
	...
	if (componentProps) {
		return (
			<Route
				{...rest}
				render={(props) =>
					!UserService.isLogged() ? <Component {...props} updateUsername={componentProps} /> : <Redirect to='/' />
				}
			/>
		);
	} else {
		return (
			<Route
				{...rest}
				render={(props) => (!UserService.isLogged() ? <Component {...props} /> : <Redirect to='/' />)}
			/>
		);
	}
};
					...
					<Suspense fallback={<div>Loading</div>}>
						...
						<Switch>
							...
							<PrivateRoute
								...
								componentProps={this.updateUsername}
							/>
