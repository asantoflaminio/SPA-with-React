...
class EditPublication extends React.Component {
	...
	loadProvinces(province, city) {
		...
		LocationService.getProvinces().then(function(response) {
			...
			for (let i = 0; i < response.data.length; i++) {
				if (response.data[i].province === province) provinceid = response.data[i].provinceid;
			}
	...
	loadCities(provinceid, city) {
		...
		if (provinceid !== undefined) {
			LocationService.getCities(provinceid).then(function(response) {
				...
				for (let i = 0; i < response.data.length; i++) {
					if (response.data[i].city === city) cityid = response.data[i].cityid;
				}
	...
	reInitializeForm() {
		...
		this.state.coveredFloorSize === '-1'
			? (schema.coveredFloorSize = '')
	...
	render() {
		...
		const neighborhood = this.state.neighborhoods.map(function(item) {
			return (
				<option value={item.neighborhoodid} key={item.neighborhood + item.neighborhoodid}>
					{' '}
					{item.neighborhood}{' '}
				</option>
			);
								...
								<Form onSubmit={(event) => handleSubmit(event) || this.handleFormSubmit(event, errors)}>
									<div className='sub_box'>
										...
										<Form.Group as={Col} md='12'>
											...
											<Form.Control
												...
												onChange={(event) =>
													this.updateNeighborhoodValue(event, values) && handleChange(event)
												}
												onBlur={handleBlur}
												value={values.neighborhoodid}
												isInvalid={!!errors.neighborhoodid && touched.neighborhoodid}>
												<option disabled value=''>
													{t('publish.neighborhoodHolder')}
												</option>
												{neighborhood}
											</Form.Control>
											<Form.Control.Feedback type='invalid'>{errors.neighborhoodid}</Form.Control.Feedback>
										</Form.Group>
										<Form.Group as={Col} md='12'>
									...
									<div className='sub_box'>
										...
										<Form.Group as={Col} md='12'>
											...
											/>
											<Form.Control.Feedback type='invalid'>{errors.dimention}</Form.Control.Feedback>
										</Form.Group>
										<Form.Group as={Col} md='12'>
											...
											/>
											<Form.Control.Feedback type='invalid'>{errors.coveredFloorSize}</Form.Control.Feedback>
										</Form.Group>
										<Form.Group as={Col} md='12'>
											...
											/>
											<Form.Control.Feedback type='invalid'>{errors.parking}</Form.Control.Feedback>
										</Form.Group>
										<Form.Group as={Col} md='12'>
											...
											/>
											<Form.Control.Feedback type='invalid'>{errors.balconies}</Form.Control.Feedback>
										</Form.Group>
										<Form.Group as={Col} md='12'>
