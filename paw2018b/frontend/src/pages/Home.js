...
class HomeReal extends React.Component {
			...
			for (let i = 0; i < loopEnd; i++) {
				...
				table.push(
					<div key={array[i].publicationid}>
						<HomeCard publication={array[i]} ready={this.setReady} index={index} />
					</div>,
				);
			}
		}
	...
	setReady() {
		if (
			LocalStorageService.getCounter() ===
			this.state.publicationsSale.length + this.state.publicationsRent.length
		) {
			LocalStorageService.deleteCounter();
							...
							<form>
								<div className='search_list-home'>
									<fieldset className='search_list-container-home rounded'>
										<div
											className='search_list-item selected'
											id='buy'
											onClick={() => this.setOperation('FSale')}>
											<input value='FSale' type='radio' />
											<label id='buy-label'>{t('home.buy')}</label>
										</div>
										<div className='search_list-item' id='rent' onClick={() => this.setOperation('FRent')}>
											<input value='FRent' type='radio' />
											<label id='rent-label'>{t('home.rent')}</label>
										</div>
									</fieldset>
								</div>
								<div id='icons'>
													...
													<div className='card-body row no-gutters'>
														...
														<div className='col'>
															<select
																...
																onChange={(e) => this.setPropertyType(e)}>
					...
					<div>
						<section className='newest_homes'>
							...
							<div>
								...
								<div>
									<ul
										id='newest-homes-fsale'
										className={
											this.state.loading === true ? 'hidden newest-homes-list' : 'newest-homes-list'
										}>
										{tableSale}
									</ul>
								</div>
