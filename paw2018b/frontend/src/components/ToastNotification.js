...
class ToastNotification extends React.Component {
	...
	handleClose() {
		let showAgain = true;
		if (this.props.oneTimeShow !== undefined) showAgain = false;

		if (this.props.specialCloseModal === true) this.props.closeFunction();
	...
	handleShow() {
		...
		if (this.props.checkModal === false) {
			timerid = setTimeout(() => {
				this.setState({show: false, oneTimeShow: showAgain});
			}, 7000);
		...
		if (this.props.checkModal === true) {
			return (
				<>
					...
					<Button
						variant='primary'
						onClick={() => this.props.acceptFunction(this.props.functionParameter) & this.handleClose()}>
						{t('modal.accept')}
					</Button>
				</>
			);
