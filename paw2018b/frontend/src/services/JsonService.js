const JsonService = (function() {
	function _getFormValues(target) {
		...
		for (let i = 0; i < target.length; i++) {
			if (
				(target[i].type === 'radio' && target[i].checked === false) ||
				target[i].type === 'submit' ||
				target[i].type === 'button' ||
				target[i].type === 'file'
			)
				continue;
			else if (target[i] instanceof HTMLOptionElement) {
				if (target[i].selected === false) continue;
				else array.push(new JsonObject(target[i].parentElement.name, target[i].value));
			} else {
	...
	function _getJSONParsed(array) {
		let jsonData = array;
		if (array.constructor.name !== 'Array') jsonData = _getFormValues(array);
