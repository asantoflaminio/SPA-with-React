...
const UserService = (function() {
	...
	function _isLogged() {
		if (LocalStorageService.getAuthorization() != null) return true;
		else return false;
	}

	function _isAdmin() {
		let role = LocalStorageService.getRole();
		if (role.includes(ROLE_ADMIN)) return true;
		else return false;
	}

	function _isLocked() {
		let role = LocalStorageService.getRole();
		if (role !== undefined && role.includes(ROLE_LOCKED)) return true;
		else return false;
	}
	...
	async function _getUsers(queryParameters) {
		return await axios({
			...
			.catch(function(error) {
				return axios.isCancel(error) ? error : error.response;
			});
	}
