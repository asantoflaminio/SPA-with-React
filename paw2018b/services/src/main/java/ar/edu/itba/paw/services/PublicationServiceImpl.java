...
public class PublicationServiceImpl implements PublicationService {
	...
	public Publication create(String title, String address, String neighborhood, String city, String province,
		...
		return publicationDao.create(title, address, neighborhood, city, province, operation, price, description,
	...
	public boolean editData(String title, String address, String neighborhood, String city, String province,
		...
		return publicationDao.editData(title, address, neighborhood, city, province, operation, price, description,
				propertyType, bedrooms, bathrooms, floorSize, parking, coveredFloorSize, balconies, amenities, storage,
				expenses, publicationid);
	...
	public PublicationDTO transform(Publication publication) {
		...
		publicationDTO = new PublicationDTO(publication.getPublicationid(), publication.getTitle(),
				publication.getProvince().getProvince(), publication.getCity().getCity(),
				publication.getNeighborhood().getNeighborhood(), publication.getAddress(), publication.getOperation(),
				publication.getPrice().toString(), publication.getDescription(), publication.getPropertyType(),
				publication.getBedrooms().toString(), publication.getBathrooms().toString(),
	...
	public List<Filter> generateFilters(String operation, String propertyType, Integer minPrice, Integer maxPrice,
		...
		addStringFilter(filters, operation, Constants.DataBaseFilterName.OPERATION, Constants.QueryFilterName.OPERATION,
				Constants.QueryOperator.EQUAL);
		addStringFilter(filters, propertyType, Constants.DataBaseFilterName.PROPERTYTYPE,
				Constants.QueryFilterName.PROPERTYTYPE, Constants.QueryOperator.EQUAL);
		addIntegerFilter(filters, minPrice, Constants.DataBaseFilterName.PRICE, Constants.QueryFilterName.MINPRICE,
				Constants.QueryOperator.GREATER_OR_EQUAL);
		addIntegerFilter(filters, maxPrice, Constants.DataBaseFilterName.PRICE, Constants.QueryFilterName.MAXPRICE,
				Constants.QueryOperator.LESS_OR_EQUAL);
		addIntegerFilter(filters, minFloorSize, Constants.DataBaseFilterName.FLOORSIZE,
				Constants.QueryFilterName.MINFLOORSIZE, Constants.QueryOperator.GREATER_OR_EQUAL);
		addIntegerFilter(filters, maxFloorSize, Constants.DataBaseFilterName.FLOORSIZE,
				Constants.QueryFilterName.MAXFLOORSIZE, Constants.QueryOperator.LESS_OR_EQUAL);
		addIntegerFilter(filters, bedrooms, Constants.DataBaseFilterName.BEDROOMS, Constants.QueryFilterName.BEDROOMS,
				Constants.QueryOperator.EQUAL);
		addIntegerFilter(filters, bathrooms, Constants.DataBaseFilterName.BATHROOMS,
				Constants.QueryFilterName.BATHROOMS, Constants.QueryOperator.EQUAL);
		addIntegerFilter(filters, parking, Constants.DataBaseFilterName.PARKING, Constants.QueryFilterName.PARKING,
				Constants.QueryOperator.EQUAL);
		if (!locked)
			addBooleanFilter(filters, locked, Constants.DataBaseFilterName.LOCKED, Constants.QueryFilterName.LOCKED,
					Constants.QueryOperator.EQUAL);
