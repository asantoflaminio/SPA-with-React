...
public class PublicationHibernateDao implements PublicationDao {
	...
	public Publication create(String title, String address, String neighborhood, String city, String province,
		...
		final Publication pub = new Publication(title, address, operation, Integer.valueOf(price), description,
				propertyType, Integer.valueOf(bedrooms), Integer.valueOf(bathrooms), Integer.valueOf(floorSize),
				Integer.valueOf(parking), new Date(), Integer.valueOf(coveredFloorSize), Integer.valueOf(balconies),
				amenities, storage, Integer.valueOf(expenses));
	...
	public Publication findById(long id) {
		String queryString = SELECT_STATEMENT_SEARCH + "left join fetch pub.user "
	...
	public boolean editData(String title, String address, String neighborhood, String city, String province,
		...
		final Query query = em.createQuery("update Publication as pub set pub.title = :title, "
				+ "pub.address = :address, " + "pub.operation = :operation, pub.price = :price, "
				...
				+ "pub.parking = :parking, " + "pub.coveredFloorSize = :coveredFloorSize, "
				+ "pub.balconies = :balconies, " + "pub.amenities = :amenities, " + "pub.storage = :storage, "
				+ "pub.expenses = :expenses " + "where pub.publicationid = :publicationid");
	...
	public void updateLockUserPublications(boolean lock, long publicationid, long userid) {
		final Query query = em.createQuery(
				"update Publication as pub set pub.locked = :locked where pub.publicationid = :publicationid");
	...
	public Map<Integer, Long> getSimpleFilter(List<Filter> filters, String address, String filterName) {
		...
		countClause = "select new ar.edu.itba.paw.models.FilterCountQuery(COUNT(pub." + filterName + "), pub."
				+ filterName + ") " + SELECT_STATEMENT_FILTER;
	...
	public void addLocationFilter(List<Filter> filters, String address, Map<String, Long> locationMap,
		...
		final String hqlString;
		final TypedQuery<FilterCountQuery> queryCount;
		countClause = "select new ar.edu.itba.paw.models.FilterCountQuery(COUNT(pub." + location + "), pub." + location
				+ "." + location + ") " + SELECT_STATEMENT_FILTER;
		whereStatement = addFiltersStatement(whereStatement, filters, address);
		groupByClause = "GROUP BY pub." + location + ",pub." + location + "." + location + " ORDER BY pub." + location
				+ " ASC";
		hqlString = countClause + whereStatement + groupByClause;
		queryCount = em.createQuery(hqlString, FilterCountQuery.class);
		addFiltersValues(queryCount, filters, address);
	...
	public String addFiltersStatement(String query, List<Filter> filters, String address) {
		...
		for (int i = 0; i < filters.size(); i++) {
			...
			if (i == 0)
				query += " pub." + current.getDataBaseNameValue().getDataBaseFilterName() + " "
						+ current.getOperator().getQueryOperator() + " :" + current.getNameValue().getQueryFilterName()
						+ " ";
			else
				query += " AND pub." + current.getDataBaseNameValue().getDataBaseFilterName() + " "
						+ current.getOperator().getQueryOperator() + " :" + current.getNameValue().getQueryFilterName()
						+ " ";
		}
